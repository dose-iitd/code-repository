...
class Trainer(object):
    def __init__(self, args):
        ...
        #{'params': model.parameters(), 'lr': args.lr},
        #{'params': pretrained_dict[adv_class_one].parameters(), 'lr': args.lr},
        #{'params': model_dict['adv_class_two'].parameters(), 'lr': args.lr},
        #{'params': model_dict.adv_class_three.parameters(), 'lr': args.lr},
        ], lr=args.lr, momentum=0.9, weight_decay=args.weight_decay)
        optimizer1 = torch.optim.SGD(model.parameters(), lr=0.007, momentum=0.9, weight_decay=args.weight_decay)
    ...
    def test_time_training(self,trainer,dataloaders, epoch,loader):
        ...
        for i, (inputs, labels,filepath) in enumerate(loader):
            ...
            output2 = output * weights_prob
            #output = interp(output2).cpu().data[0].numpy()
            output3 = interp(output2)
            #output1 = interp(output2).cpu().data.numpy()
            #softmax_entropy
            ...
            #print(miou)
            #print('C-Driving cloudy mIOU')
            #miou=trainer.validation_nd(epoch,dataloaders['valset_c_driving_cloudy'])
            #print(miou)
            #print('C-Driving overcast mIOU')
            #miou=trainer.validation_nd(epoch,dataloaders['valset_c_driving_overcast'])
            #print(miou)
