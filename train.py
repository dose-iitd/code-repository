...
class ModelWithBNPrototypes(nn.Module):
    def __init__(self, original_model):
        super(ModelWithBNPrototypes, self).__init__()
        self.features = nn.Sequential(*list(original_model.children())[:-2])  # assuming the classifier is the last layer
        self.classifier = list(original_model.children())[-1]
        self.bn_prototypes = {}  # Dictionary to store the BN prototypes
    ...
    def compute_bn_prototypes(self, x):
        prototypes = []
        for module in self.features.modules():
            if isinstance(module, nn.BatchNorm2d):
                module.eval()  # Set to eval mode to use running statistics
                with torch.no_grad():
                    _ = module(x)  # Forward pass to update the running statistics
                prototypes.append((module.running_mean, module.running_var))
                module.train()  # Set back to train mode
        self.bn_prototypes = prototypes  # Store the prototypes for later use
        return prototypes

# Instantiate the original model, e.g., ResNet
original_model = models.resnet18(pretrained=True)

# Create student and teacher models
student_model = ModelWithBNPrototypes(original_model)
teacher_model = ModelWithBNPrototypes(original_model)

# Load pre-trained weights into teacher model
# Assuming 'teacher_state_dict.pth' is the path to the saved teacher weights
teacher_state_dict = torch.load('teacher_state_dict.pth')
teacher_model.load_state_dict(teacher_state_dict)

# Example usage of the models
images = torch.randn(4, 3, 224, 224)  # Batch of 4 images, 3 channels, 224x224 pixels each

# Forward passes
student_output = student_model(images)
teacher_output = teacher_model(images)

# Compute BN prototypes
student_bn_prototypes = student_model.compute_bn_prototypes(images)
teacher_bn_prototypes = teacher_model.compute_bn_prototypes(images)
def cross_entropy_loss(input, target):
    return F.cross_entropy(input, target)

def layer_distillation_loss(student_outputs, teacher_outputs, student_prototypes, teacher_prototypes):
    loss = 0.0
    for student_output, teacher_output, student_prototype, teacher_prototype in zip(student_outputs, teacher_outputs, student_prototypes, teacher_prototypes):
        loss += F.mse_loss(student_output, teacher_output)
    return loss / len(student_outputs)

def domain_distillation_loss(student_prototypes, domain_prototypes):
    loss = 0.0
    for student_prototype, domain_prototype in zip(student_prototypes, domain_prototypes):
        loss += F.mse_loss(student_prototype, domain_prototype)
    return loss / len(student_prototypes)

def compute_total_loss(student_model, teacher_model, images, targets, domain_prototypes, lambda1, lambda2):
    # Forward pass for student and teacher models
    student_outputs = student_model(images)
    student_prototypes = student_model.compute_bn_prototypes(images)  # Assume we have a method to compute BN prototypes
    teacher_outputs = teacher_model(images)
    teacher_prototypes = teacher_model.compute_bn_prototypes(images)  # Assume we have a method to compute BN prototypes
    
    # Compute losses
    ce_loss = cross_entropy_loss(student_outputs, targets)
    layer_distill_loss = layer_distillation_loss(student_outputs, teacher_outputs, student_prototypes, teacher_prototypes)
    domain_distill_loss = domain_distillation_loss(student_prototypes, domain_prototypes)

    # Combine losses
    total_loss = ce_loss + lambda1 * layer_distill_loss + lambda2 * domain_distill_loss
    return total_loss
...
class Trainer(object):
    ...
    def test_time_training(self,trainer,dataloaders, epoch,loader):
        ...
        def priority_fn (image, label, output):
            ...
            with torch.no_grad():
                # Example usage
                # Assume `student_model` and `teacher_model` are instances of some neural network models
                # `images` is a batch of input images, and `targets` is the corresponding ground truth labels
                # `domain_prototypes` is the prototype for the current domain
                # `lambda1` and `lambda2` are the hyperparameters for the loss terms

                # Compute the loss
                lambda1 = 0.1
                lambda2 = 1.0
                domain_aware_batch_loss = compute_total_loss(student_model, teacher_model, images, targets, domain_prototypes, lambda1, lambda2)            # if self.args.cuda:
            return -float(domain_aware_batch_loss.item())
        ...
        for iteration in range (self.args.num_iter):
            ...
            for i, (inputs, labels,filepath) in enumerate(tbar):
                ...
                student_model = ... # Define or load a model
                teacher_model.train()
                with torch.no_grad():
                    output = teacher_model (image.cuda()).cpu()
                ...
                for i, (inputs, labels,filepath) in enumerate(loader):
                    ...
                    if self.args.densencloss ==0:
                        ...
                        lambda1 = 0.1
                        lambda2 = 1.0
                        domain_aware_batch_loss = compute_total_loss(student_model, teacher_model, images, targets, domain_prototypes, lambda1, lambda2)            # if self.args.cuda:                        if self.args.cuda:
                        domain_aware_batch_loss = domain_aware_batch_loss.cuda()
                        #loss = 0.000001*celoss.cuda() + domain_aware_batch_loss
                        loss = domain_aware_batch_loss
                        #train_ncloss += domain_aware_batch_loss.item()
